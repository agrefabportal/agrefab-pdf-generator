...
async function testPdfGenerator_createsFile() {
    ...
    pdf.saveGuide().then(deleteFile(pdf.filePath));
}
/**
...
async function testFileSystemNamingConvention() {
    ...
    assert.equal(pdf1.filePath, 'Untitled.pdf', `Default file name did not match expected. PDF file name: ${pdf1.fileName} and file path: ${pdf1.fileName}`);
    ...
    pdf2.saveGuide().then(deleteFile(pdf2.filePath));
}
/**
...
function deleteFile(filePath) {
    ...
    console.log(`📄🗑 ${filePath} was created and deleted. Comment out @function deleteFile() to preserve the files for visual inspection.`)
}